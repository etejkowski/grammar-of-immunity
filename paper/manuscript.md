...
1. **No comparison to published predictors.** Our baseline is an internal k-mer model, not NetTCR, ERGO-II, TCRdist3, GLIPH2, TEINet, or protein-language-model embeddings, and we do not evaluate on the official IMMREP25 test set. Between-arm comparisons are internally controlled but absolute performance is not situated against the state of the art.
2. **Negatives are shuffled pairs**, not experimentally verified non-binders, a design the IMMREP post-mortems identify as inflating performance. Absolute AUCs are optimistic.
...
13. Shugay M, et al. VDJdb: a curated database of T-cell receptor sequences with known antigen specificity. `https://github.com/antigenomics/vdjdb-db`.
